...
No need to install NodeJs manually, it relies on a Java native binding of https://deno.com/[Deno, window="_blank"] and https://esbuild.github.io[esbuild, window="_blank"]. For libraries, all the NPM catalog is accessible through Maven or Gradle dependencies.
...
With Gradle, you need to add this plugin and the dependency:
...
dependencies {
    implementation("io.quarkiverse.web-bundler:quarkus-web-bundler:{project-version}")
}
----
...
NOTE: By default `{#bundle /}` inserts both script and style tags for `app`, this is xref:advanced-guides.adoc#bundle-tag[configurable].
