...
Additional ignored files may be specified using the `regex:` or `glob:` prefix to indicate whether the value should be interpreted as a regular expression or a glob pattern.

These entries are added to the default set of ignored files, so the defaults do not need to be repeated unless customization is required.
...
These defaults are intended to be extended via `ignored-files`, which allows additional patterns to be configured, but if needed they can also be overriden.
