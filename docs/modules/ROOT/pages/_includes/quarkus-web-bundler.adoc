...
[.description]
--
If enabled, scan a local directory in the project root for web assets (e.g. `project-root/web/`). This is separate from classpath resources which are scanned automatically.


ifdef::add-copy-button-to-env-var[]
Environment variable: env_var_with_copy_button:+++QUARKUS_WEB_BUNDLER_LOCAL_WEB_DIR+++[]
endif::add-copy-button-to-env-var[]
ifndef::add-copy-button-to-env-var[]
Environment variable: `+++QUARKUS_WEB_BUNDLER_LOCAL_WEB_DIR+++`
endif::add-copy-button-to-env-var[]
--
|boolean
|`+++true+++`
...
These entries are added to the default set of scanner ignored files `quarkus.project-scanner.default-ignored-files`, so the defaults do not need to be repeated unless customization is required.
