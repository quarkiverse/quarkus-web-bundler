...
IMPORTANT: This rendering is happening at build time, so you won't be able to access any Qute runtime data in this file. If you want to data, you should add the Qute Web extension (or https://quarkiverse.github.io/quarkiverse-docs/quarkus-renarde/dev/[Renarde extension] for MVC with Qute) and use `src/main/resrouces/templates/` directory instead. The <<bundle-tag>> will also work in this case.
